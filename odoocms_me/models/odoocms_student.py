...
class OdooCMSStudent(models.Model):
    ...
    code = fields.Char( store=True, tracking=True)
    # compute = '_get_code'
    ...
    street2 = fields.Char()
    street = fields.Char()
    per_state_id = fields.Char()
    remarks = fields.Char('remarks')
    paper_attachment = fields.Char('paper_attachment')
    topic = fields.Char('topic')
    award_date = fields.Char('award_date')
    ...
    # tag_ids = fields.Many2many('odoocms.student.tag',  'student_id', string='Group/Tag')
    # 'student_tag_rel', 'tag_id',

    partner_id = fields.Many2one('res.partner', 'Partner', required=True, ondelete="cascade")
    #
    career_id = fields.Many2one('odoocms.career', 'Career', readonly=True, )
    # states = {'draft': [('readonly', False)]}
    program_id = fields.Many2one('odoocms.program', 'Academic Program', tracking=True, readonly=True, )
    # states = {'draft': [('readonly', False)]}
    ...
    campus_id = fields.Many2one('odoocms.campus', 'Campus',  store=True)
    ...
    #     states={'draft': [('readonly', False)]})
    study_scheme_id = fields.Many2one('odoocms.study.scheme', 'Study Scheme',  store=True)
    # compute = '_get_study_scheme',
    minor_scheme_id = fields.Many2one('odoocms.study.scheme', 'Minor Scheme', tracking=True, readonly=True, )
    # states = {'draft': [('readonly', False)]}
    term_id = fields.Many2one('odoocms.academic.term', 'Current Academic Term', tracking=True, readonly=True, )
    # states = {'draft': [('readonly', False)]}
    session_id = fields.Many2one('odoocms.academic.session', 'Academic Session', tracking=True, readonly=True, )
    # states = {'draft': [('readonly', False)]}
    academic_ids = fields.One2many('odoocms.student.academic', 'student_id','Academic')
...
#     name = fields.Char(string='Student Name',  readonly=True, store=True)
#     # related = 'student_id.name'
#     message_from = fields.Char(string='Message From', readonly=True)
#     program_id = fields.Many2one('odoocms.program', string='Program',  readonly=True, store=True)
...
# class OdooCMSStudentDocuments(models.Model):
...
#     prc_scaned_copy = fields.Binary()
#     inter_scaned_copy = fields.Binary()
#     cnic_scanned_copy = fields.Binary()
#     dae_first_year = fields.Binary()
#     dae_second_year = fields.Binary()
#     dae_third_year = fields.Binary()
#     test_certificate = fields.Binary()
#     cnic_back_scanned_copy = fields.Binary()
#
#     student_id = fields.Many2one('odoocms.student', string='Program',  store=True)
#     # application_id = fields.Many2one('odoocms.student', string='Application', store=True)
#     comment = fields.Html(string="Comment", required=True)
#     date = fields.Date('Comment Date', default=date.today(), readonly=1)
