...
class OdooCMSDiscipline(models.Model):
    ...
    programss_ids = fields.One2many('odoocms.program','discipline_id','Academic Programs')
...
class OdooCMSInstituteLine(models.Model):
    _name = 'odoocms.institute.line'
    _description = 'CMS Institute line'
...
class OdooCMSInstitute(models.Model):
    ...
    parent_id = fields.Many2one('odoocms.institute', string='Parent Institute')
    parent_name = fields.Char( readonly=True, string='Parent name')
...
class OdooCMSCareer(models.Model):
    ...
    improve_course_limit = fields.Integer(string='improve course limit')
